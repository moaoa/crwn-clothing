...
import {
  ...
  addItemSuccess,
  addItemFailure,
  decreaseItemQuantitySuccess,
  decreaseItemQuantityFailure,
} from "../actions/actionCreatores";
import {
  addItem as addItemUtil,
  decreaseItemQuantity as decreaseItemQuantityUtil,
} from "../reducers/utils/cartUtils";
...
export function* addItemAsync(action) {
  try {
    const { userData, userRef } = yield getUserData();
...
export function* decreaseItemQuantityAsync(action) {
  try{
    const { userData, userRef } = yield getUserData();
    const newCartItems = yield decreaseItemQuantityUtil(
    userData.cartItems || [],
    action.itemId
  );
  yield console.log(newCartItems);
    yield userRef.update({ ...userData, cartItems: newCartItems });
    yield put(decreaseItemQuantitySuccess(newCartItems));
  }catch(err){
   yield put(decreaseItemQuantityFailure(err))
  }
}
